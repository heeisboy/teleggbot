...
def start_bot():
    ...
    def handler_call(call):
        ...
        if call.data in func.list_product():
            ...
            if len(row) == 0:
                bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
                    text='К сожалению, данный товар закончился, ожидайте пополнения',
                    reply_markup=menu.main_menu
                )
        ...
        if call.data == 'card_balance':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=settings.card_balance,
                reply_markup=menu.repenish_balance,
                parse_mode="html"
            )
        ...
        if call.data == 'purchases':
            ...
            if len(msg) == 0:
                bot.edit_message_text(chat_id=chat_id,
                                      message_id=message_id,
                                      text='Вы еще не совершали покупок в нашем магазине!',
                                      reply_markup=menu.main_menu)
 ...
 #card
        if call.data == 'card_balance':
            bot.edit_message_text(
                ...
                message_id=message_id,
                text=settings.card_balance,
                reply_markup=menu.card_balance,
                parse_mode="html"
            )


        if call.data == 'renish_balance':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=settings.renish_balance,
                reply_markup=menu.renish_balance,
                parse_mode="html"
            )
        ...
        if call.data == 'replenish_balance':
            bot.edit_message_text(chat_id=chat_id,
                                  ...
                                  reply_markup=menu.replenish_balance, parse_mode="html")
        ...
        if call.data == 'referral_web':
            ...
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f'👥 <b>Реферальная сеть</b>\n\n'
                     f'Ваша реферельная ссылка:\n'
                     f'<b>https://t.me/{settings.bot_login}?start={ref_code}\n\n</b>'
                     f'За все время вы заработали - <b>{func.check_all_profit_user(chat_id)} ₽\n\n</b>'
                     f'<i>Если человек, приглашенный по вашей реферальной ссылке, пополнит баланс, то вы получите {settings.ref_percent} % на баланс от суммы его депозита</i>',
                reply_markup=menu.main_menu,
                parse_mode='html'
            )
    ...
    def buy_2(message):
        try:
            ...
            bot.send_message(chat_id=message.chat.id,
                             text='⚠️Что-то пошло не по плану',
                             reply_markup=menu.main_menu)
