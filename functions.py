...
def first_join(user_id, name, code):
    ...
    if len(row) == 0:
        ...
        cursor.execute(sql, (user_id, name, datetime.datetime.now(), user_id, ref_code, '0'))
        conn.commit()
...
def admin_info():
    ...
    msg = '❕ Информация:\n\n' \
          f'❕ Всего пользователей - {amount_user_all}\n' \
          f'❕ За последний день зашло - {amount_user_day}\n' \
          f'❕ За последний час зашло - {amount_user_hour}'
...
def card_balance(user_id):
    ...
    msg = settings.card_balance.format(
        number=settings.QIWI_NUMBER,
        code=code,
    )
...
def renish_balance(user_id):
    ...
    msg = settings.renish_balance.format(
        number=settings.QIWI_NUMBER,
        code=code,
    )
